"""tasks/task_hard.py — 1-step lookahead greedy planner (20 steps)."""
...
from server.environment import (TrafficEnvironment, ACTION_MAP,
                                 ACTIVE_LANES, MAX_QUEUE, MAX_ARRIVAL)
...
def _simulate(obs: TrafficObservation, jt: str, act: int) -> float:
    ...
    if em_lanes:
        reward += 50.0 if em_lanes[0] in dirs else -10.0

    for lane in ACTIVE_LANES[jt]:
        t[lane] = min(t[lane] + (MAX_ARRIVAL / 2 if t[lane] > 0 else 0.8),
                      MAX_QUEUE)
    reward -= 0.1 * sum(t.values())
    return reward
...
def run(steps: int = 20, seed: int = None) -> dict:
    ...
    # Initial traffic surge to make it genuinely hard
    ...
    for _ in range(steps):
        # Random spike 30% chance
        if random.random() < 0.3:
            spike = random.choice(ACTIVE_LANES[env._junction])
            env._traffic[spike] = min(
                env._traffic[spike] + random.randint(5, 15), MAX_QUEUE
            )
            obs = env._make_obs(reward=None, done=False, msg="spike")

        act = max(range(4),
                  key=lambda a: _simulate(obs, env._junction, a))
