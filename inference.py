...
# ── OpenAI client (required — all LLM calls must use this) ─────────────
client = OpenAI(
...
def get_llm_action(obs: dict, step: int) -> tuple[int, str]:
    """Ask the LLM to choose an action. Returns (action_id, reason)."""
    ...
    try:
        ...
        # Fallback: try to find a digit in the response
        action_id = 0
        reason    = f"Parse fallback. Raw: {content[:100]}"
        for char in content:
            if char in "0123":
                action_id = int(char)
                break
...
def main():
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if em_active:
            em_total += 1
            # Check if the served direction had emergency
            served_dirs = {
